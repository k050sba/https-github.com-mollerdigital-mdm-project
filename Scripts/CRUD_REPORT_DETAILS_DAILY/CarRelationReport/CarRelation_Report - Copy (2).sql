...
and CREATE_DATE>DATEADD(month, DATEDIFF(month, -1, getdate()) - 1, 0) 
and CREATE_DATE<GETDATE()
and exists(select 1 from  MDM_Custom..C_REL_PARTY_CAR_HIST_CR_REPORT where C_REL_PARTY_CAR.ROWID_OBJECT=C_REL_PARTY_CAR_HIST_CR_REPORT.ROWID_OBJECT and C_REL_PARTY_CAR_HIST_CR_REPORT.LAST_ROWID_SYSTEM='BRING')
--and (RELATION_END_DATE is not null )
and role_code  in('B','L') union all 
select count(1),DATEADD(month, DATEDIFF(month, -1, getdate()) -2, 0) as date_of_month  from C_REL_PARTY_CAR 
where exists(select 1 from C_B_PARTY where C_B_PARTY.ROWID_OBJECT=C_REL_PARTY_CAR.PARTY_ID and COUNTRY_CODE='NO')
...
and CREATE_DATE>DATEADD(month, DATEDIFF(month, -1, getdate()) - 1, 0) 
and CREATE_DATE<GETDATE()
and exists(select 1 from  MDM_Custom..C_REL_PARTY_CAR_HIST_CR_REPORT where C_REL_PARTY_CAR.ROWID_OBJECT=C_REL_PARTY_CAR_HIST_CR_REPORT.ROWID_OBJECT and C_REL_PARTY_CAR_HIST_CR_REPORT.LAST_ROWID_SYSTEM='DATAFACTORY')
--and (RELATION_END_DATE is not null )
and role_code  in('B','L') union all 
select count(1),DATEADD(month, DATEDIFF(month, -1, getdate()) -2, 0) as date_of_month  from C_REL_PARTY_CAR 
where exists(select 1 from C_B_PARTY where C_B_PARTY.ROWID_OBJECT=C_REL_PARTY_CAR.PARTY_ID and COUNTRY_CODE='NO')
