...
insert into MDM_CUSTOM..SAP_MDM_DEARLE_TOPID (VAT_NUMBER,top_id)
 ...
 and org.party_id in (select ROWID_OBJECT from MDM_CM..C_B_PARTY where  HUB_STATE_IND=1 and COUNTRY_CODE='NO' and TYPE='O' and LAST_ROWID_SYSTEM='SAP') 
...
where
exists(select 1 from C_B_PARTY where org.PARTY_ID=C_B_PARTY.ROWID_OBJECT and HUB_STATE_IND=1 and COUNTRY_CODE='NO' and LAST_ROWID_SYSTEM='SAP'
...
and TOP_ID not in (select TOP_ID from C_B_MDM_SAP_ORGANIZATION where TYPE='DO');