...
and exists( select 1 from C_B_PARTY_STATUS sts where sts.PARTY_ID=prty.ROWID_OBJECT and sts.STATUS_TYPE in('A','I')and RELATION_END_DATE is null and sts.HUB_STATE_IND=1)
...
where prty.HUB_STATE_IND=1 and prty.TYPE in('O') 
and prty.COUNTRY_CODE in('NO','LT','LV','EE')
and exists( select 1 from C_B_PARTY_STATUS sts where sts.PARTY_ID=prty.ROWID_OBJECT and sts.STATUS_TYPE='L' and sts.RELATION_END_DATE is null and sts.HUB_STATE_IND=1)
and prty.ROWID_OBJECT in(select PARTY_ID from C_B_ORGANIZATION where (ALIAS_NAME is not null or LEGAL_NAME is not null or ORGANIZATION_NUMBER is not null) )
...
where prty.HUB_STATE_IND=1 and prty.TYPE in('P') 
and prty.COUNTRY_CODE in('NO','LT','LV','EE')
and exists( select 1 from C_B_PARTY_STATUS sts where sts.PARTY_ID=prty.ROWID_OBJECT and sts.STATUS_TYPE in('A','I') and sts.RELATION_END_DATE is null and sts.HUB_STATE_IND=1  )
and exists(select 1 from C_B_PERSON per where per.PARTY_ID=prty.ROWID_OBJECT and per.FIRST_NAME is not null and per.SURNAME is not null and per.HUB_STATE_IND=1)
and (exists(select 1 from C_B_PHONE phone where phone.PARTY_ID=prty.ROWID_OBJECT and phone.PHONE_NUMBER is not null and phone.HUB_STATE_IND=1)
and exists(select 1 from C_REL_PARTY_ADDRESS rela where rela.PARTY_ID=prty.ROWID_OBJECT and rela.ADDRESS_TYPE='ST' and rela.HUB_STATE_IND=1))
group by COUNTRY_CODE, TYPE
) as A
